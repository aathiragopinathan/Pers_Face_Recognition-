...
from capture_manager import capture_embeddings_for_person
from display_utils import draw_face_info, TempFace
from api_client import send_recognition_event, send_registration_event  # ADD THIS
from config import *

# ADD THESE HELPER FUNCTIONS
def notify_frontend(event_type, data):
    """Send notification to FastAPI server (frontend communication)"""
    try:
        url = f"http://{FASTAPI_HOST}:{FASTAPI_PORT}/{event_type}"
        response = requests.post(url, json=data, timeout=2)
        return response.status_code == 200
    except Exception as e:
        print(f"[FRONTEND_API] Communication error: {e}")
        return False

def check_user_registration_decision():
    """Check if user made registration decision via frontend"""
    try:
        url = f"http://{FASTAPI_HOST}:{FASTAPI_PORT}/get_user_response"
...
def main():
    print("=" * 80)
    print("🏢 ENHANCED FACE RECOGNITION SYSTEM WITH API COMMUNICATION")
    print("🎯 Motion Detection + Distance Validation + Frontend Communication")
    ...
    try:
        while True:
            ...
            # Draw stats
            info_lines = [
                f"Face Recognition + Motion + Distance Validation + API Communication",  # UPDATED
            ...
            for i, line in enumerate(info_lines):
                font_size = 0.6 if i == 0 else 0.5  # UPDATED font size
                cv2.putText(frame, line, (10, 30 + i*20), cv2.FONT_HERSHEY_SIMPLEX, font_size, (255, 255, 255), 2)
            ...
            if faces:
                ...
                if face is None:
                    ...
                    if frame_count % 3 == 0:
                        ...
                        if person_id:
                            # Known person - ADD API COMMUNICATION
                            last_face_info = {
                                "type": "recognized",
                                "person_id": person_id,
                            ...
                            if person_id != last_detected_id or (current_time - last_detection_time) > 2:
                                ...
                                # ADD: Notify frontend
                                notify_frontend("person_detected", {
                                    "person_id": person_id,
                                    "confidence": similarity,
                                    "is_unknown": False,
                                    "message": f"Welcome back, Person {person_id}!"
                                })
                                
                                # ADD: Send to external API
                                send_recognition_event(person_id, similarity)
                            ...
                            if should_register:
                                ...
                                # ADD: Notify frontend and wait for user decision
                                notify_frontend("person_detected", {
                                    "person_id": None,
                                    "confidence": 0,
                                    "is_unknown": True,
                                    ...
                                    if has_response:
                                        if should_register_user:
                                            ...
                                            if registration_successful:
                                                ...
                                                # ADD: Send to external API
                                                send_registration_event(person_id)
...
if __name__ == '__main__':
    # ADD: Start FastAPI server in a separate thread
